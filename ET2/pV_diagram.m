...
% arbete per cykel
W_cycle = trapz(V2, p2); %"numerical integration via the trapezoidal method" :P
W_cycle = W_cycle * 1000; %från kJ till J
...
Qin = sum(p_mid(dV > 0) .* dV(dV > 0)) * 1000 * f;   % J

% Qout: sum over compression (dV < 0)
Qout = -sum(p_mid(dV < 0) .* dV(dV < 0)) * 1000 * f; % J
